...
def check_columns_in_files(df, vae_feature_columns=vae_feature_columns, metadata_columns=metadata_columns):
    """
    Check if all necessary columns are present in the dataframe.

    Args:
        df (pd.DataFrame): DataFrame to check.
        vae_feature_columns (list): List of VAE feature columns.
        metadata_columns (list): List of metadata columns.
    """
    # Check for missing VAE feature columns
    missing_vae_cols = set(vae_feature_columns) - set(df.columns)
    if missing_vae_cols:
        print(f"Missing VAE feature columns: {missing_vae_cols}")
    
    # Check for missing metadata columns
    missing_metadata_cols = set(metadata_columns) - set(df.columns)
    if missing_metadata_cols:
        print(f"Missing metadata columns: {missing_metadata_cols}")
...
def drop_empty_columns(df):
    """
    Drop columns that are completely empty (i.e., all NaN values).

    Args:
        df (pd.DataFrame): Input dataframe.

    Returns:
        df (pd.DataFrame): Dataframe with empty columns dropped.
    """
...
def flag_columns_with_missing_data(df, threshold=0.25):
    """
    Flag columns that have more than 25% missing data.

    Args:
        df (pd.DataFrame): Input dataframe.
        threshold (float): The threshold for missing data (default: 0.25).
        
    Returns:
        flagged_columns (list): Columns with more than the specified threshold of missing data.
    """
    missing_data_percent = df.isnull().mean()
    flagged_columns = missing_data_percent[missing_data_percent > threshold].index.tolist()
    if flagged_columns:
        print(f"Columns with more than 25% missing data: {flagged_columns}")
...
def preprocess_pipeline(data_folder="data/AMP_AD_MSBB_MSSM", scale_method = "standard", visualize = False, aggregate_by_gene = False, return_stats = False, plot_path = None, return_metadata = False, save_path = False):
    ...
    flagged_columns = flag_columns_with_missing_data(df)
